...
bx = [900, 400, True]   #resolution of box, then if to show or not
...
def getlist(x1,y1,x2,y2,time):
    ...
    for x in range(0,time):         #setting up boarders
        ...
        lx.append(random.uniform(-100,x1+50))
        ly.append(random.uniform(-100,win.height+100))
        bx.append(random.uniform(-300,win.width+300))
        by.append(random.uniform(-100,y2+50))
        tx.append(random.uniform(-300,win.width+300))
        ty.append(random.uniform(y1-75,win.height+100))
        rx.append(random.uniform(x2-50,win.width+100))
...
def getxy(x=-1, y=-1, t=-1):
    ...
    if t == -1:
        ...
        return [x1, y1, x2, y2], getlist(x1,y1,x2,y2,t)
...
def udate(x):
    ...
    if count == uplimf:
        lol = nlol
        #nlol = randlist(lol)
        nlol = getlist(scnl[0], scnl[1], scnl[2], scnl[3], t)
